import { isAuthConfigured, login, sessionCookieOptions, SESSION_COOKIE } from "@/lib/auth";
...
export async function POST(request: Request) {
  const rl = rateLimit(getClientKey(request), { limit: 10, windowMs: 60_000 });
  ...
  if (!token) return errorResponse("Invalid credentials.", 401);
  return Response.json(
    { ok: true },
