...
/* ═══════════════════════════════════════════════════════════════════════
   CLIENT-SIDE DETECTION (useSyncExternalStore, no setState-in-effect)
   ═══════════════════════════════════════════════════════════════════════ */

const emptySubscribe = () => () => {};
function getIsClientSnapshot() { return true; }
function getIsClientServerSnapshot() { return false; }
    ...
    <>
      {parts.map((part, i) =>
        part.highlight ? (
          <mark
            key={i}
            className="bg-transparent font-semibold text-[var(--foreground)] underline decoration-2 underline-offset-2"
          >
            {part.text}
          </mark>
        ) : (
...
export function CommandPaletteProvider({ children }: { children: ReactNode }) {
  ...
  // ── Client-side detection (no setState-in-effect) ──────────
  const isClient = useSyncExternalStore(emptySubscribe, getIsClientSnapshot, getIsClientServerSnapshot);

  // ── Search index (initialized in state, safe for useMemo) ───
  ...
  const [, setAnalytics] = useState<Analytics>({ searches: [], clicks: [] });
  ...
  useEffect(() => {
    ...
    if ("Worker" in window) {
      try {
        ...
        worker.postMessage({ type: "buildIndex", docs: searchIndex });
      } catch {
    ...
    // Load persistence
    void Promise.all([loadRecent(), loadUsage(), loadFavorites(), loadAnalytics()]).then(
      ([r, u, f, a]) => {
        startTransition(() => {
          setRecent(r);
          setUsage(u);
  ...
  // isSearching is derived: true when query differs from debouncedQuery
  ...
  // Query change handler — resets active index without effect
      ...
      if (worker) {
        const handleMessage = (e: MessageEvent) => {
          if (e.data.type === "searchResults") {
            const results = e.data.results as ScoredDoc[];
            setScoredResults(results);
            const ai = generateAIAnswer(
              q,
              results.map((r) => r.doc),
            );
            setAIAnswer(ai);
  ...
  }, [debouncedQuery, isClient, favorites, usage, recent, searchIndex]);
  ...
  const recordUsage = useCallback((doc: SearchDoc, queryStr: string) => {
    setUsage((prev) => {
      ...
      if (existing) {
        next = prev.map((u) =>
          u.id === doc.id ? { ...u, count: u.count + 1, ts: Date.now() } : u,
        );
      } else {
        next = [
          ...prev,
          { id: doc.id, label: doc.label, href: doc.href, group: doc.group, ts: Date.now(), count: 1 },
        ];
      }
    ...
    setRecent((prev) => {
      ...
      const next = [
        { id: doc.id, label: doc.label, href: doc.href, group: doc.group, ts: Date.now() },
        ...filtered,
      ].slice(0, 8);
  ...
  useEffect(() => {
    const handleKeyDown = (event: globalThis.KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case "PageDown": {
          event.preventDefault();
          setActive((p) => Math.min(p + 5, maxIndex));
          return;
        }
        case "PageUp": {
          event.preventDefault();
          setActive((p) => Math.max(p - 5, 0));
          return;
        }
        case "Enter": {
          event.preventDefault();
        ...
        case "Tab": {
          event.preventDefault();
          if (event.shiftKey) setActive((p) => (p <= 0 ? maxIndex : p - 1));
          else setActive((p) => (p >= maxIndex ? 0 : p + 1));
          return;
        }
      }
    };
  ...
  const suggestions = useMemo(() => {
    ...
    return [];
  }, [debouncedQuery, totalItems, searchIndex]);
          ...
          >
            ...
            >
              ...
              <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-[var(--border)] bg-[var(--card)] px-4 py-3.5">
                {isSearching ? (
                  <Loader2 className="h-[18px] w-[18px] shrink-0 animate-spin text-[var(--muted)]" aria-hidden />
                ) : (
                  <Search className="h-[18px] w-[18px] shrink-0 text-[var(--muted)]" aria-hidden />
                )}
                ...
                <input
                  ...
                  aria-label="Search Pedumo"
                  aria-controls="cmdk-listbox"
                  aria-activedescendant={
                    totalItems > 0 ? `cmdk-item-${scoredResults[active]?.doc.id}` : undefined
                  }
                  autoComplete="off"
                  autoCorrect="off"
                  autoCapitalize="off"
              ...
              {query.startsWith(">") && (
                <div className="border-b border-[var(--border)] bg-[var(--background-subtle)]/50 px-4 py-2 text-xs text-[var(--muted)]">
                  <span className="font-medium text-[var(--foreground)]">Command mode:</span> Type a
                  command to execute directly. Examples:{" "}
                  <code className="rounded bg-[var(--background)] px-1">&gt;book</code>{" "}
                  <code className="rounded bg-[var(--background)] px-1">&gt;contact</code>
                </div>
              )}
              {query.startsWith("@") && (
                <div className="border-b border-[var(--border)] bg-[var(--background-subtle)]/50 px-4 py-2 text-xs text-[var(--muted)]">
                  <span className="font-medium text-[var(--foreground)]">Scoped search:</span>{" "}
                  Searching within{" "}
                  <code className="rounded bg-[var(--background)] px-1">
                    {query.slice(1).split(" ")[0]}
                  </code>
                </div>
              )}
              ...
              >
                ...
                {totalItems === 0 && suggestions.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-3 px-4 py-12 text-center">
                    <Search className="h-8 w-8 text-[var(--muted)] opacity-40" aria-hidden />
                    <div>
                  ...
                  <div className="px-2 py-4">
                    <p className="px-3 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--muted)]">
                      Did you mean
                    </p>
                    {suggestions.map((item) => (
                      ...
                      >
                        <Lightbulb className="h-4 w-4 shrink-0 text-[var(--muted)]" aria-hidden />
                  ...
                  grouped.map((section) => (
                    <section key={section.group} className="mb-1" aria-label={section.group}>
                      <div className="flex items-center justify-between px-3 py-2">
                        ...
                        {section.group === "Favorites" && (
                          <Star className="h-3 w-3 text-[var(--muted)]" aria-hidden />
                        )}
                        {section.group === "Frequently Used" && (
                          <TrendingUp className="h-3 w-3 text-[var(--muted)]" aria-hidden />
                        )}
                        {section.group === "Recent" && (
                          <Clock className="h-3 w-3 text-[var(--muted)]" aria-hidden />
                        )}
                      </div>
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              isActive
                                ...
                                : "hover:bg-[var(--background-subtle)]/40",
                            )}
                          >
                            {/* Number shortcut */}
                            {globalIdx < 9 && (
                              <span
                                className={cn(
                                  "hidden h-5 w-5 shrink-0 items-center justify-center rounded font-mono text-[10px] font-medium sm:flex",
                                  isActive
                                    ? "bg-[var(--foreground)]/15 text-[var(--foreground)]"
                                    : "bg-[var(--background-subtle)] text-[var(--muted)]",
                                )}
                              >
                                {globalIdx + 1}
                              </span>
                            ...
                            <div
                              className={cn(
                                ...
                                isActive
                                  ...
                                  : "bg-[var(--background-subtle)] text-[var(--muted)] group-hover:text-[var(--foreground)]",
                              )}
                            >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {item.detail && (
                                <p className="mt-0.5 truncate text-[11px] text-[var(--muted)] opacity-70">
                                  {item.detail}
                                </p>
                              )}
                            </div>
                            ...
                            >
                              <Star className={cn("h-3.5 w-3.5", isFav && "fill-amber-400")} aria-hidden />
                            </span>
                            ...
                            <ArrowRight
                              className={cn(
                                ...
                                isActive
                                  ? "translate-x-0 text-[var(--foreground)] opacity-100"
